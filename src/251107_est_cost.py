...
MONTHLY_FEE = 15.0      # New: flat monthly fee in dollars
...
def append_monthly_fee_rows(df: pd.DataFrame, monthly_fee: float = MONTHLY_FEE) -> pd.DataFrame:
    ...
    for p in months:
        ...
        fee_rows.append({
            ...
            'sell_price': 0.0,
            'cost': monthly_fee,
            # Optional convenience fields if present in df:
            'month': ts.month if 'month' in df.columns else None,
            'hour': ts.hour if 'hour' in df.columns else None,
        })

    fee_df = pd.DataFrame(fee_rows)
    # Make sure columns align; missing columns will be added automatically
...
if __name__ == '__main__':
    ...
    for kw in kws:
        ...
        fig_path = f'../figs/kw_{kw}/251107_lineplot_monthly_cost_kw_{kw}.png'
        monthly_costs = {}

        for s in scenarios:
            batt_file = f'{base_dir}251107_merged_battery_2025_onward_{s}.csv'
            output_file = f'{base_dir}251107_electricity_costs_{s}.csv'
            ...
            plt.savefig(f'../figs/kw_{kw}/251107_lineplot_pv_{s}.png')

            # Aggregate monthly cost and print total (fee rows are included automatically)
    ...
    costs.to_csv('../output/251107_all_costs.csv')
    # plt.show()
