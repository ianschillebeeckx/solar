...
def add_ev_weekly_charging(
    ...
    prorate_partial_weeks: bool = True,
) -> pd.DataFrame:
    ...
    week_anchor : str
        Used only when ``prorate_partial_weeks=False`` to define weekly grouping.
    prorate_partial_weeks : bool
        If True (default), allocate a fixed kWh per eligible hour equal to
        ``weekly_kwh / (len(charge_days)*len(charge_hours))`` so partial weeks
        get a proportional share (fixes Dec 31 edge case). If False, normalize
        within each observed week so that every observed week totals exactly
        ``weekly_kwh``.
    """
    df = hourly.copy()
    ...
    # Build eligibility frame from the active slice
    ...
    # Initialize additions (one-to-one with 'sub')
    add = np.zeros(len(sub), dtype=float)

    if prorate_partial_weeks:
        # Constant kWh per eligible hour across all weeks
        hours_full_week = max(len(charge_days) * len(charge_hours), 1)
        per_hour_kwh = weekly_kwh / hours_full_week
        add[eligible.values] = per_hour_kwh
    else:
        # Legacy behavior: enforce exactly 'weekly_kwh' per observed week
        sub2 = sub.copy()
        sub2["DAY"] = sub2["DATE"].dt.normalize()
        sub2["WEEK"] = sub2["DAY"].dt.to_period(week_anchor)

        hours_per_week = (
            sub2.assign(eligible=eligible)
                .groupby("WEEK")["eligible"].sum()
                .replace(0, np.nan)
        )
        per_hour_map = (weekly_kwh / hours_per_week).to_dict()

        idx_elig = eligible.values
        weeks = sub2.loc[idx_elig, "WEEK"].values
        if len(weeks) > 0:
            per_hour_vec = np.array([per_hour_map.get(w, 0.0) for w in weeks], dtype=float)
            add[idx_elig] = per_hour_vec

    # Apply back to the main DataFrame
...
if __name__ == "__main__":
    ...
    RAW_CSV = Path("../output/pge_usage_2025_projected_251107.csv")
    OUT_CSV = Path("../output/251107_2025_adjusted_kWh.csv")
    PLOT_PNG = Path("../figs/251107_original_vs_adjusted.png")
    PLOT_PNG_HOD = Path("../figs/251107_hour_of_day_profiles.png")
