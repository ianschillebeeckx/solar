...
USAGE_2025_FP = Path("../input/pge_usage_2024-11-01_to_2025-10-31.csv")
...
# --- Load combined usage (electric + gas) for Nov 2024 – Oct 2025 ---
...
# --- Apply mapping to fill any missing 2025 months by copying other 2025 months from the mapped periods ---
months_2025_actual = sorted(
    set(idx for idx in monthly_elec.index if idx.year == 2025).union(
        idx for idx in monthly_gas_2025_actual.index if idx.year == 2025
    )
)
...
for month_num in range(1, 13):
    period = pd.Period(f"2025-{month_num:02d}", freq="M")
    if period not in months_2025_actual:
        targets_2025.append(period)
proj_elec_2025 = {}
proj_gas_2025 = {}
map_2024_to_2025 = {}
...
# Generate projections for any missing 2025 months
...
OUT_FP = Path("../output/pge_usage_2025_projected_251107.csv")
...
plt.title("2025 Electricity (kWh): Observed months plus mapped projections")
plt.xlabel("Month")
plt.ylabel("kWh")
...
plt.title("2025 Gas (therms): Observed months plus mapped projections")
plt.xlabel("Month")
plt.ylabel("therms")
plt.xticks(rotation=45)
plt.tight_layout()
